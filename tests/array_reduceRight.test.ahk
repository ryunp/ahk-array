group := tester.newGroup("array_reduceRight(array, callback, initialValue:='__NULL__')")
...
group.newTest("Sum a property of all objects"
    , Assert.equal(143, array_reduceRight(complex_array, func("objProp_addition").bind("age"), 0)))

group.newTest("Copy a string property of all objects into an array"
    , Assert.arrayEqual(names, array_reduceRight(complex_array, func("objProp_arrayPush").bind("name"), [])))

group.newTest("Concat nested arrays"
    , Assert.arrayEqual([4, 5, 3, 4, 1, 2], array_reduceRight(nested_arrays, func("reduce_nestedArray"))))
=======
